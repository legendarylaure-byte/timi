...
- **Backend: composer intent consumer** (`main.py`): new `tiktok_composer_job()` (APScheduler, every 5 min) drains `tiktok_composer` Firestore collection for `status='queued'` intents, resolves the video file (checkpoint `pipeline_checkpoints/{video_id}` `video_path` first, then R2 presigned download via `_resolve_video_for_publish`), calls `multi_platform_publish(... platforms=['tiktok'], tiktok_privacy_level=..., toggles...)`, and marks the intent `published`/`failed` with `publish_id`/URL.
- **Dashboard API**: `GET /api/tiktok/composer/options` — pulls `privacy_level_options` + `comment_disabled`/`duet_disabled`/`stitch_disabled`/`vid_private`/`display_privacy` from TikTok `/v2/post/publish/creator_info/query/` using the Firestore-stored `platform_settings/tiktok` access token. `POST /api/tiktok/composer/publish` — rejects empty `privacy_level` (no default), writes the intent doc to `tiktok_composer`.
- **Dashboard UI**: "Compose TikTok Post" card added to `dashboard/src/app/dashboard/publishing/page.tsx` — video picker (recent unpublished `videos` via client SDK), title, **privacy dropdown with "Select privacy…" placeholder, no default selected**, Comments/Duet/Stitch toggles **default off**, submit queues the post. Flow: compose → intent doc → backend consumer publishes within ~5 min.
- **Deploy**: thin overlay image built on `timi-pipeline:latest` (COPY changed `main.py` + `multi_platform_publisher.py`), container recreated, healthy. Dashboard committed to trigger Vercel build.
- **AGENTS note**: TikTok PUBLIC still gated by the external Direct Post audit (user completing the form); remains `SELF_ONLY` until audit grants, then flip `TIKTOK_PRIVACY_LEVEL=PUBLIC_TO_EVERYONE` in `.env`/Firestore + recreate.

### D28-2: TikTok Public Go-Live Prep — Chunked Upload Fix + Token Refresh + SELF_ONLY interim (deployed live)
- **TikTok chunked upload fixed (root cause of all 09-01 TikTok failures)**: `_upload_tiktok` (`multi_platform_publisher.py`) previously always sent `chunk_size=file_size, total_chunk_count=1` → TikTok rejected every file >64MB with `400 invalid_params "chunk size is invalid"`. Now splits per Media Transfer guide: `chunk_size = min(file_size, 64MB)`, `total_chunk_count = floor(file_size/chunk_size)` (with a halve-down guard while <2 chunks), and **sequential PUTs where the FINAL chunk absorbs ALL trailing bytes** (`end = file_size-1`, up to 128MB) — the 416-on-last-chunk bug was caused by capping every chunk at `chunk_size` and never sending the remainder. Verified the boundary math against TikTok's 50,000,123-byte doc example (5 chunks, last = 10,000,123B). Now init→chunked PUT (206/200 per chunk)→201→status-poll publish works.
- **PUBLIC blocked externally (no code fix)**: with chunking fixed, `<64MB` file init returns `403 unaudited_client_can_only_post_to_private_accounts` — the **Direct Post app audit is not yet granted** in the TikTok portal (portal still shows "To lift these restrictions, apply for an audit"). Code defaults `TIKTOK_PRIVACY_LEVEL` to `PUBLIC_TO_EVERYONE` but the USER DECISION (option b) is to run `SELF_ONLY` in `.env` until the audit grants, then flip back to `PUBLIC_TO_EVERYONE`.
- **Token refresh + persistence**: a 09-01 token expired within ~24h → `401 access_token_invalid`. Refreshed via `_refresh_tiktok_token()` (grant_type=refresh_token) and wrote fresh `TIKTOK_ACCESS_TOKEN/REFRESH_TOKEN/OPEN_ID` to **both** `agents/.env` + root `.env` AND Firestore `env_vars`/`platform_settings/tiktok`. Verified Firestore has NO `TIKTOK_PRIVACY_LEVEL` (won't override `.env` SELF_ONLY at boot sync).
- **Deploy method (avoided 3GB pip rebuild)**: full `docker compose build` re-downloaded the whole torch/CUDA stack (`python:3.11-slim` base tag invalidation) and died on a pip read-timeout after ~1h. Switched to a **thin overlay image** (`FROM timi-pipeline:latest` + COPY the one changed file) → built in ~1s. Recreated container, healthy.
- **Verification (init probes)**: PUBLIC 40MB → 403 unaudited (audit gate); SELF_ONLY 40/93/166MB → 200 + upload_url (chunk math accepted). Then **2 surviving long videos backfilled + published to TikTok (SELF_ONLY)**: "Introduction to Python for Beginners" + "Nepal death toll crosses 1,000..." (both `success=True`; Firestore `videos.*.tiktok_url`/`tiktok_status:published` recorded). SELF_ONLY posts return no public id → URL falls back to `publish_id` (`v_pub_file~...`), expected.
- **3 shorts NOT backfillable**: their rendered files lived in the ephemeral compositor temp dir that was cleaned on container recreate; `videos.*.video_path`=None, only live YouTube URLs remain. Will be regenerated by tonight's 15:05 UTC run on the fixed TikTok path.
- **Flipping the switch back**: when the TikTok Direct Post audit grants, set `TIKTOK_PRIVACY_LEVEL=PUBLIC_TO_EVERYONE` in `agents/.env` + root `.env` (and optionally Firestore `env_vars`) and recreate the container.

### D28: Operational Health + Exact-Duration + Speed + Agent Improvements (2026-09-01)
- **TikTok production OAuth re-authorization complete**: tokens for `legendarylaure` stored in `platform_settings/tiktok` AND Firestore `env_vars` (`TIKTOK_ACCESS_TOKEN`/`OPEN_ID`/`REFRESH_TOKEN`), synced at container boot. Pipeline publishes YouTube + TikTok.
- **Dead alerting revived (`get_pipeline_metrics`)**: `agents/utils/firebase_status.py` now defines `get_pipeline_metrics()` (reads `pipeline_metrics` collection) — this single function reactivates the whole daily alert block in `main.py:daily_analytics_job` (`check_pipeline_health_alert` + `check_staleness`) that was silently dying on a missing-symbol ImportError swallowed by a bare `except`. Now the health + staleness alerts actually fire.
- **Slack-first alerts (`alert_manager.py:send_alert`)**: default channel switched from dead Telegram (`bot/notifications.py` doesn't exist) to Slack (working, `SLACK_WEBHOOK_URL` set). Added `SLACK_WEBHOOK_URL` to `.env.example`.
- **New operational alerts** (`daily_analytics_job`): (1) system resource warning from heartbeat (disk ≥85% or memory ≥90%), (2) missed-slot / daily-volume guard — alerts if fewer than `SCHEDULE_SHORTS_PER_DAY + SCHEDULE_LONG_PER_DAY` videos published in the last 26h, with per-format breakdown.
- **Log rotation (`main.py:_get_log_file`)**: `logs/pipeline.log` now auto-rotates at 10MB (keeps `.log.1`) so container logs are greppable, not a single unbounded file.
- **Exact shorts duration fixed (root cause)**: `SHORTS_MAX_DURATION` was `45` in `.env` AND `crew/scriptwriter.py` hard-coded shorts to "MAXIMUM 60s / AT MOST 150 words" (`ponytail:` comment), ignoring `max_duration`. Fixed: `.env`→`180`, compositor trim default→`180`, and scriptwriter now scales shorts narration+scene-count to the configured cap (`short_cap`, ~2.2 wps, 1 scene/25s).
- **Exact longs duration fixed**: `crew/scriptwriter.py` long branch used `target_words = max(1200, ...)` which forced 8-min scripts even for a 300s target. Now `max(360, max_duration*2.5)` so regular longs (300s→~750 words) and deep lessons (600s→1500 words) both match their target. Storyboard long scene guidance aligned to scriptwriter's 12-18 scenes.
- **Speed: parallel upscaler (`utils/upscaler.py`)**: frame upscaling now runs via `ThreadPoolExecutor` (up to 8 workers) instead of a serial per-frame loop — the biggest render bottleneck, now ~4-8× faster.
- **Speed: persistent LTX cache (`models/ltx_model.py`)**: cache dir changed from lost-on-recreate `tempfile.gettempdir()/ltx_cache` to project-local, env-overridable `LTX_CACHE_DIR` (default `agents/tmp/ltx_cache`), now mounted as a Docker volume — daily repeat prompts hit cache across restarts.
- **Docker compose** (`docker-compose.yml`): added `./agents/tmp/ltx_cache/:/app/tmp/ltx_cache/` volume.
- **Agents quality**: all agents route to `OLLAMA_MODEL=qwen2.5:7b` (kept explicit in `OLLAMA_MODEL_ROUTES`, evaluators off the weaker gemma3:4b) with automatic Gemini fallback. `crew/title_optimizer.py` now passes `agent_id="title_optimizer"` to `get_llm()`. Title length hard-capped at 100 chars at the single choke point `_pick_best_title` (`len>100 → word-boundary truncate`) — LLMs can't count chars; YouTube truncates long titles. Director "fix" now feeds back to a scriptwriter re-author pass (`_rewrite_on_director_fix`), then re-simplifies — safe fallback to original script on any failure. Deleted dead agents `crew/composer.py` + `crew/metadata.py` (never called; wasted LLM calls per video).
- **Skipped (noted)**: single-pass ffmpeg compose (merging ~5 re-encode passes into one filter graph) — high regression risk to the working compositor, deferred; parallel upscaler + cache persistence deliver the main time win today.

### TikTok Production Go-Live (TikTok app approved 2026-09-01)
- **TikTok app approved** for production posting. Sandbox replaced by production client `awb0qex84vm6ii4z` / secret (production secret kept OUT of git — only in `.env`, `agents/.env`, Firestore `env_vars`, container env).
- **Keys locations**: `agents/.env`, `.env`, Firestore `env_vars/TIKTOK_CLIENT_KEY` + `TIKTOK_CLIENT_SECRET` (set via gitignored `agents/scripts/set_tiktok_prod.py`). NEVER commit real keys; `.env*` + `set_tiktok_prod.py` are gitignored.
- **TikTok enabled**: `PLATFORMS_TO_PUBLISH=youtube,tiktok` in `agents/.env` + `.env.example`. IG/FB still disabled (Meta app deleted).
- **`_upload_tiktok` fix** (`agents/utils/multi_platform_publisher.py`): now uses the working status-poll flow — init (with `post_info.privacy_level` in body) → PUT upload (with `Content-Range: bytes 0-{size-1}/{size}` header) → poll `/v2/post/publish/status/fetch/` until `PUBLISH_COMPLETE`. The old `/v2/post/publish/video/publish/` POST no longer returns an id / 404s for some clients, so we poll status instead. `video_id` falls back to `publish_id` when no public post id is returned (private/SELF_ONLY posts).
- **OAuth tokens persist to pipeline env**: dashboard callback (`dashboard/src/app/api/auth/tiktok/callback/route.ts`) now writes `TIKTOK_ACCESS_TOKEN`, `TIKTOK_OPEN_ID`, `TIKTOK_REFRESH_TOKEN` to BOTH `platform_settings/tiktok` AND Firestore `env_vars/` so the pipeline picks them up via `sync_env_from_firestore` at boot.
- **Privacy**: production default `PUBLIC_TO_EVERYONE` (`TIKTOK_PRIVACY_LEVEL` env can override). The sandbox demo script (`tiktok_demo_publish.py`, gitignored) still defaults to `SELF_ONLY` for test posts.
- **OAuth state**: dashboard + CLI both use `timi_prod` (was `sandbox_timi`/`timi_sandbox`). Scopes `user.info.basic,video.publish,video.upload`.
- **Deploy**: image `timi-pipeline:latest` rebuilt (`b480bff33784`), container recreated, healthy. Verified in-container: status-polling + Content-Range in `_upload_tiktok`, production keys synced from Firestore (31 env vars), `PLATFORMS_TO_PUBLISH=youtube,tiktok`.
...
- **Model routing** (`utils/llm_helper.py` + crew files): `OLLAMA_MODEL_ROUTES` env (JSON `{agent_id: model_tag}`) is explicit but all agents default to **`OLLAMA_MODEL=qwen2.5:7b`**; **Gemini is the automatic fallback** on empty/invalid LLM responses (via `empty_response_fallback`). Crew files pass `agent_id=` into `get_llm()` so any agent is routable. `verify_ollama_model()` accepts an optional `model` arg.
...
- **Deploy**: `ollama pull gemma3:4b` on host; `timi-pipeline:latest` rebuilt; container recreated (healthy). In-container verified: both models reachable via `host.docker.internal:11434`, all agents→qwen2.5:7b, Gemini fallback armed, scheduler clean.
