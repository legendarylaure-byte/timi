      ...
      setParticles((prev) =>
        prev.map((p) => {
          ...
          if (dist < 150) {
            return {
              ...p,
              size: Math.min(p.size + 0.5, 6),
              opacity: Math.min(p.opacity + 0.2, 0.8),
            };
          }
          return {
            ...p,
            size: Math.max(p.size - 0.1, 1),
            opacity: Math.max(p.opacity - 0.05, 0.2),
          };
        })
      );
    };
        ...
        >
          ...
          <div className="relative z-10 p-8 sm:p-10">
            ...
            >
              ...
              <p className="text-light-muted dark:text-dark-muted text-sm">Sign in to continue your creative journey</p>
            </motion.div>
            ...
            <div className="flex items-center gap-4 my-8">
              ...
              <span className="text-xs text-light-muted dark:text-dark-muted uppercase tracking-wider">Secure Auth</span>
              <div className="flex-1 h-px bg-gradient-to-l from-transparent to-white/10" />
            </div>

            {/* Security badges */}
            <motion.div className="flex items-center justify-center gap-6 text-xs text-light-muted dark:text-dark-muted">
              {['Google Auth', 'Encrypted', 'Secure'].map((item, i) => (
                <motion.div
                  key={item}
                ...
                >
                  <span className={`w-2 h-2 rounded-full ${['bg-light-success', 'bg-light-secondary', 'bg-light-accent'][i]}`} />
                  <span>{item}</span>
                </motion.div>
              ))}
            </motion.div>
            ...
            <p className="text-center mt-8 text-sm text-light-muted dark:text-dark-muted">
              ...
              <Link href="/signup" className="text-light-primary hover:text-light-secondary font-medium transition-colors">
                Create one
              </Link>
            </p>
          </div>
